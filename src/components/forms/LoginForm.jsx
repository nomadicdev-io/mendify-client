...
export default function LoginForm() {
    ...
    return (
      ...
      transition={{ duration: 0.3, type: 'tween' }}
        ...
        className="flex flex-col gap-6 w-[22.5rem] bg-white p-5 rounded-2xl shadow-2xl shadow-gray-300">
          ...
          <form.Field 
            name="password"
            validators={{
              onSubmit: ({ value }) => !value ? 'Password is required' : value.length < 3 ? 'Password must be at least 3 characters' : value.length > 255 ? 'Password must be less than 255 characters' :  undefined,
            }}
            children={(field) => (
              <InputField 
                label="Password"
